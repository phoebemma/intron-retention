...
| `data_new/Pre_Exercise/Relief_PreExc_splicing_data.RDS`      | ReLiEf baseline SpliceQ data      |
| `data_new/Pre_Exercise/Relief_PreExc_metadata.RDS`           | ReLiEf baseline metadata          |
| `data_new/processed_data/Relief_splicing_data.RDS`           | ReLiEf SpliceQ data               |
| `data_new/processed_data/Relief_metadata.RDS`                | ReLiEf metadata                   |
...
This builds models to analyse differentially spliced introns

```{r}

# A folder for the models
ifelse(!dir.exists(file.path("data_new", "models")), dir.create(file.path("data_new", "models")), FALSE)

source("R/Preexercise_model.R")
```

The script above produces the following files containing model outputs and the argument that produced the models

| Data                                                                   | Argument                                                                                           |
|------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|
| `data_new/models/preExc_count_interaction_model.RDS`                   | y \~ age\*sex + (1\|study) +(1\|participant), family = glmmTMB::beta_family()                      |
| `data_new/models/preExc_group_interaction_model.RDS`                   | y \~ group\*sex + (1\|study) +(1\|participant), family = glmmTMB::beta_family()                    |
| `data_new/models/preExc_group_only_model.RDS`                          | y \~ group + (1\|study) +(1\|participant), family = glmmTMB::beta_family()                         |
| `data_new/models/PreExc_count_no_interaction_model.RDS`                | y \~ age+sex + (1\|study) +(1\|participant), family = glmmTMB::beta_family()                       |
| `data_new/models/preExc_group_and_sex_without_interaction_model.RDS`   | y \~ group + sex  + (1\|study) +(1\|participant), family = glmmTMB::beta_family()                 |
| `data_new/models/Preexc_age_class_no_interaction_unique_intercept.RDS` | y \~ age_class + sex  + (1 + age_class\|study) +(1\|participant), family = glmmTMB::beta_family() |
| `data_new/models/PreExc_age_class_sex_no_interaction.RDS`              | y \~age_class + sex  + (1\|study) +(1\|participant), family = glmmTMB::beta_family()              |
